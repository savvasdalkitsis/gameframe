...
class BmpUseCase {
    ...
    fun rasterizeToBmp(bitmap: Bitmap): InputStream {
        val (w, h) = bitmap.dimensions
        val rasterByteSize = w * h * 3
        val fileSize = rasterByteSize + DATA_OFFSET

        val buffer = ByteBuffer.allocate(fileSize)
        writeHeader(buffer, fileSize, w, h, rasterByteSize)
        writeData(buffer, bitmap)
        return buffer.array().inputStream()
    }

    private fun writeHeader(buffer: ByteBuffer, fileSize: Int, w: Int, h: Int, rasterByteSize: Int) {
    ...
    private fun writeData(buffer: ByteBuffer, bitmap: Bitmap) {
        val (w, h) = bitmap.dimensions
        for (row in w downTo 1) {
            for (col in 1..h) {
                buffer.put(toRgb(bitmap.getPixelAt(col, row)))
            }
        }
    }
